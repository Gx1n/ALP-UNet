...
class ResUnet(nn.Module):
    def __init__(self, channel, filters=[64, 128, 256, 512],norm_eval=False):
    ...
    def forward(self, x):
        ...
        x4up = self.upsample_1(x4)
        #att_x3 = self.cbam_256(x3)
        x5 = torch.cat([x4up, x3], dim=1)

        x6 = self.up_residual_conv1(x5)

        x6up = self.upsample_2(x6)
        #att_x2 = self.cbam_128(x2)
        x7 = torch.cat([x6up, x2], dim=1)

        x8 = self.up_residual_conv2(x7)

        x8up = self.upsample_3(x8)
        #att_x1 = self.cbam_64(x1)
        x9 = torch.cat([x8up, x1], dim=1)

        x10 = self.up_residual_conv3(x9)

        #output = self.output_layer(x10)
        dec_outs=[x4,x6,x8,x10]
        return dec_outs
