...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.up = nn.ConvTranspose2d(in_channels, (in_channels // 2)-3, kernel_size=2, stride=2)
            self.conv = wsDoubleConv(in_channels, out_channels)

    #新增差异
    def forward(self, x1, x2, lap):
        x1 = self.up(x1)
        # input is CHW
...
class AttUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x1up = self.up1(x5, x4, lap4)
        x2up = self.up2(x1up, x3, lap3)
        x3up = self.up3(x2up, x2, lap2)
        x4up = self.up4(x3up, x1, lap1)
